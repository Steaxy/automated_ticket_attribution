...
import pytest
from app.application.send_report import (
...
class FakeReportEmailSender:
    ...
    def send_report_email(self, subject: str, body: str, attachment_path: Path) -> None:
        self.calls.append((subject, body, attachment_path))
...
def test_send_classified_requests_report(tmp_path: Path) -> None:
    ...
    subject, body, attachment_path = sender.calls[0]

    assert subject == (
        "Automation Engineer interview - technical task - John Doe"
    )
    ...
    assert attachment_path == report_file
